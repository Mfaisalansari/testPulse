 ...
 *
 * <p>Register by adding {@code com.testpulse.cucumber5} to the {@code glue}
 * array in your runner's {@code @CucumberOptions}:
 *
 * <pre>{@code
 * @CucumberOptions(
 *     glue = {"com.acme.steps", "com.testpulse.cucumber5"},
...
public class ReportingHooks {
    ...
    public void onScenarioStart(Scenario cucumberScenario) {
        try {
            ...
            String lob = Cucumber5Config.getLobResolver().resolve(cucumberScenario);
    ...
    public void captureScreenshotEarly(Scenario cucumberScenario) {
        ...
        try {
            byte[] bytes = Cucumber5Config.getScreenshotProvider().capture();
            if (bytes != null && bytes.length > 0) {
                ...
                LOG.warning("Screenshot provider returned " + (bytes == null ? "null" : "empty bytes") +
                        ...
                        "Cucumber5Config.setScreenshotProvider() during framework startup?");
            }
        } catch (Throwable t) {
     ...
     * {@code io.cucumber.java.Status} enum (changed from String in
     ...
     */
    static Status normalizeStatus(io.cucumber.java.Status cucumberStatus) {
